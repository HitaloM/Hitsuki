...
async def change_device_info(message, device, edit=False):
	...
	if 'default_bugs' not in device:
		buttons.add(
			InlineKeyboardButton("Add default bugs", callback_data=fox_change_def_bugs_cp.new(codename=codename)))
	else:
		buttons.add(
	...
	if 'default_notes' not in device:
		buttons.add(
			InlineKeyboardButton("Add default notes", callback_data=fox_change_def_notes_cp.new(codename=codename)))
	else:
		buttons.add(
...
async def change_default_notes_done(message, state, **kwargs):
	async with state.proxy() as data:
		codename = data['codename']
	mongodb.ofox_devices.update_one({'codename': codename},
	                                {'$set': {'default_notes': message.parse_entities(as_html=True)}})
	device = mongodb.ofox_devices.find_one({'codename': codename})
	await change_device_info(message, device, edit=False)
