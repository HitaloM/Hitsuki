...
from hitsuki import bot, OWNER_ID
from hitsuki.services.mongo import mongodb
from hitsuki.utils.logger import log
from hitsuki.versions import DB_STRUCTURE_VER


async def notify_bot_owner(old_ver, new_ver):
    await bot.send_message(
        OWNER_ID,
        f"Hitsuki database structure was updated from <code>{old_ver}</code> to <code>{new_ver}</code>"
    )
...
if not (data := mongodb.db_structure.find_one({'db_ver': {"$exists": True}})):
    ...
    if DB_STRUCTURE_VER > curr_ver:
        ...
        while curr_ver < DB_STRUCTURE_VER:
            ...
            log.debug("Importing: hitsuki.db." + str(new_ver))
